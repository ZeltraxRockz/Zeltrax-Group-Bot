...
from Zeltrax.modules.helper_funcs.extraction import (
    extract_user,
    extract_unt_fedban,
    extract_user_fban,
)
from Zeltrax.modules.helper_funcs.string_handling import markdown_parser
from Zeltrax.modules.disable import DisableAbleCommandHandler

import Zeltrax.modules.sql.feds_sql as sql

from Zeltrax.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
...
def new_fed(update, context):
    ...
    if len(fednam) >= 2:
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact my owner @ZeltraxRockzzz if the problem persists."
            )
            return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Zeltrax_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Zeltrax_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Zeltrax_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Zeltrax_fbanned_users.csv",
